...
with st.sidebar:
    ...
    # XP GÖSTERGESİ (Artık Hata Vermeyecek)
    ...
    # API KEY YÖNETİMİ (Otomatik ve Manuel)
    ...
    if not st.session_state['api_key']:
        ...
        api_input = st.text_input("Google API Key Gir:", type="password")
...
with tabs[1]: # KELİMELER
    ...
    if not user_words:
        if st.button("🚀 Bana Başlangıç Paketi Ekle (5 Kelime)"):
            with st.spinner("Hazırlanıyor..."):
                ...
                for w in words:
                    db.add_word(st.session_state['username'], w['word'], w['meaning'], w['example'], w['synonyms'], w['forms'])
    ...
    if user_words:
        ...
        st.dataframe(df[['word', 'meaning', 'example']], use_container_width=True)
...
with tabs[5]: # SINAV
    ...
    if 'quiz' in st.session_state:
        for i, q in enumerate(st.session_state['quiz']):
            ...
            st.radio("Seçenekler", q['options'], key=f"q{i}")
        if st.button("Bitir"):
