...
def get_db_connection():
    ...
    try:
        # 1. YÖNTEM: Bilgisayarında 'secrets.json' varsa onu kullanır
        if os.path.exists('secrets.json'):
            creds = ServiceAccountCredentials.from_json_keyfile_name('secrets.json', scope)
        
        # 2. YÖNTEM: Dosya yoksa (Bulutta ise) sistem ayarlarından okur
        elif 'GCP_JSON' in st.secrets:
            # Buluttaki gizli ayarı okuyup sözlüğe çeviriyoruz
            key_dict = json.loads(st.secrets['GCP_JSON'])
            creds = ServiceAccountCredentials.from_json_keyfile_dict(key_dict, scope)
        
        else:
            st.error("⚠️ HATA: Ne 'secrets.json' dosyası bulundu ne de Bulut ayarı (GCP_JSON) girildi.")
            return None
...
def add_word(username, word, meaning, synonyms):
    sheet = get_db_connection()
    if sheet:
        try:
